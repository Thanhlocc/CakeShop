...
public class Add extends HttpServlet {
    ...
    private void AddSanPham(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            giaNhap = 0.0; // Giá trị mặc định hoặc thông báo lỗi cho người dùng
        }
        ...
        try {
            ...
            giaBan = 0.0; // Giá trị mặc định hoặc thông báo lỗi cho người dùng
        }
        ...
        try {
            ...
            soLuong = 0; // Giá trị mặc định hoặc thông báo lỗi cho người dùng
        }

        String loaiSanPham = request.getParameter("LoaiSanPham");
        Part filePart = request.getPart("hinhAnh");
        String fileName = filePart.getSubmittedFileName(); // Lấy tên file
        String folderPath = "image/shop/"; // Đường dẫn tới thư mục chứa hình ảnh trong ứng dụng của bạn
        String filePath = folderPath + fileName; // Tạo đường dẫn hoàn chỉnh tới hình ảnh

        SanPhamDAO spdao = new SanPhamDAO();
        boolean success = spdao.insertSanPham(tenSanPham, giaNhap, giaBan, filePath, soLuong, loaiSanPham);

        if (success) {
            response.sendRedirect("success"); // Chuyển hướng đến trang thành công
        } else {
            response.sendRedirect("error"); // Chuyển hướng đến trang lỗi
        }
