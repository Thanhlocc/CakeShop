...
public class SanPhamDAO {
    ...
    public int getMaLoaiSanPham(String loaiSanPham) {
        MysqlConnect mysqlConnect = new MysqlConnect();
        int maLoaiSanPham = -1;

        try {
            String query = "SELECT MaLoaiSanPham FROM loaisanpham WHERE TenLoaiSanPham = ?";
            ...
            if (rs.next()) {
                maLoaiSanPham = rs.getInt("MaLoaiSanPham");
            }
    ...
    public boolean insertSanPham(String tenSanPham, double giaNhap, double giaBan, String filePath, int soLuong, String loaiSanPham) {
        MysqlConnect mysqlConnect = new MysqlConnect();
        SanPhamDAO spdao = new SanPhamDAO();
        int maLoaiSP = spdao.getMaLoaiSanPham(loaiSanPham);
        String query = "INSERT INTO SanPham VALUES (?, ?, ?, ?, ?, ?, ?)";
        conn = mysqlConnect.openConnection();
        try {
            PreparedStatement preparedStatement = conn.prepareStatement(query);
            preparedStatement.setString(1, tenSanPham);
            preparedStatement.setDouble(2, giaNhap);
            preparedStatement.setDouble(3, giaBan);
            preparedStatement.setString(4, filePath);
            preparedStatement.setInt(5, soLuong);
            preparedStatement.setInt(6, maLoaiSP);
    ...
    public boolean updateSanPham(int maSanPham, String tenSanPham, double giaNhap, double giaBan, String filePath, int soLuong, int loaiSanPham) {
        MysqlConnect mysqlConnect = new MysqlConnect();

        String query = "UPDATE SanPham SET TenSanPham=?, GiaNhap=?, GiaBan=?,"
                + " HinhAnh=?, SoLuong=?, MaLoaiSanPham=? WHERE MaSanPham=?";
        ...
        try {
            ...
            preparedStatement.setString(4, filePath);
            preparedStatement.setInt(5, soLuong);
            preparedStatement.setInt(6, loaiSanPham);
            preparedStatement.setInt(7, maSanPham);
            preparedStatement.executeUpdate();
            return true;
        } catch (Exception e) {
            e.printStackTrace();
